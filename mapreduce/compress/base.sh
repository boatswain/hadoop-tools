...
function recursive_list_hdfs_path() {
    ...
    if [ $? -ne 0 ];then
        LOG_INFO "RECURSIVE LIST HDFS PATH" "recursive list failed, exits..."
    ...
    if [ $FILE_NUM -le 0 ];then
        LOG_INFO "RECURSIVE LIST HDFS PATH" "recursive list input file num is 0, exits..."
        SEND_MAIL "RECURSIVE LIST HDFS PATH ERROR" "recursive list input file num is 0." ${MAIL_LIST}
        LOG_SEPERATOR
        exit 1
    fi
    ${HADOOP_BIN} fs -put $2 $3/$2
    if [ $? -ne 0 ];then
