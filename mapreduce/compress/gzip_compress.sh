...
HADOOP_TMP_DIR="/tmp/gzip_compress/"
LOCAL_LSR_TMP_FILE="gzip_compress_lsr_tmp_$RANDOM"
MAPRED_OUTPUT_DIR="/tmp/gzip_compress/${LOCAL_LSR_TMP_FILE}_output"
TODAY=`date "+%Y-%m-%d"`
...
function usage() {
    echo ""
    echo "./gzip_compress.sh <zip path> <output path> [capacity] [priority]" >&2
    echo ""
    exit 1
}
...
${HADOOP_HOME}/bin/hadoop jar ${HADOOP_HOME}/contrib/streaming/hadoop-streaming-v2.1-0.20.205.0.jar \
    -D mapred.job.name="gzip_compress-${TODAY}" \
    ...
    -mapper "./gzip_compress_mapper.sh" \
    -reducer "NONE" \
    -file "gzip_compress_mapper.sh" \
    -file "core-site.xml" \
    -cmdenv zip_input_path=${INPUT_PATH} -cmdenv zip_output_path=${OUTPUT_PATH}
ret_code=$?
if [ ${ret_code} -ne 0 ];then
    echo "`date '+%Y-%m-%d %H:%M:%S'` run compress job failed."
    echo "`date '+%Y-%m-%d %H:%M:%S'` run compress job failed. retcode=${ret_code}" | mail -s "run compress job failed." ${MAIL_LIST}
    exit 1
else
    echo "`date '+%Y-%m-%d %H:%M:%S'` run compress job successful"
fi
