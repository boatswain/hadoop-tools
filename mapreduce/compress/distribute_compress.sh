...
# mapred job name
JOB_NAME="distribute_compress-$(date '+%Y%m%d%H%M%S')"



function usage() {
    echo ""
    echo "./distribute_compress.sh <input path> <output path> <delete source> [compress type] [compress format] [mapred job capacity] [mapred job priority]" >&2
...
LOG_INFO "distribute compress" "compress type: $COMPRESS_TYPE"
...
delete_source
LOG_INFO "main" "distribute compress completed success."
LOG_SEPERATOR
