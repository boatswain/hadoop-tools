...
do
    ...
    if [ $? -eq 0 ];then
        echo "${uncompress_output_path} already exists, detele it."
        ${HADOOP_BIN} fs -rm ${uncompress_output_path}
    fi
    ${HADOOP_BIN} fs -cat ${file_path} | gzip -d | ${HADOOP_BIN} fs -put - ${uncompress_output_path}
